...
export default async function InternalLinksSection() {
  ...
  try {
    ...
    programLinks = programs.map((p) => ({ label: p.display_name, href: `/programs/${p.route_slug}` }));
  } catch {
          ...
          {columns.map((column) => (
            <Grid item xs={6} sm={4} md={2} key={column.title}>
              <Typography
                ...
                variant="caption"
                fontWeight={700}
                sx={{ color: tokens.muted, display: "block", mb: 1.25, fontSize: 12, "&:hover": { color: tokens.ink } }}
              >
