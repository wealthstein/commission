...
export async function sendOtp(phoneNumber, businessName) {
  ...
  if (!res.ok || !data.pinId) {
    // Don't hide what Termii actually said behind a generic fallback -
    // that's exactly what made this hard to diagnose. Check the field
    // names their error responses are documented to use, and fall back to
    // the full raw response rather than a made-up generic string if none
    // of them match.
    const realError = data.message || data.error || data.errors || JSON.stringify(data);
    console.error("Termii sendOtp failed:", res.status, realError);
