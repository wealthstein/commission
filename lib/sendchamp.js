 ...
 * Inactive" with no clear timeline; Sendchamp's sender IDs show Approved
 * with no pending state - including the branded "COMMISSION" sender ID,
 * approved after SC-OTP (a generic fallback) was already working.
 *
...
export async function sendOtp(phoneNumber, businessName) {
  ...
  const senderId = process.env.SENDCHAMP_SENDER_ID || "COMMISSION";
