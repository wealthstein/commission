...
export async function sendColdOutreachEmail(step, { to, companyName }) {
  const templateFn = templates[`coldOutreach${step}Template`];
  if (!templateFn) throw new Error(`No cold outreach template for step ${step}`);
  return send(to, templateFn({ companyName }));
}
