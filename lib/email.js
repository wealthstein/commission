 ...
 *
 * Brevo has been removed entirely - it's no longer used anywhere in this
 * app. commission.ng and reply.commission.ng are both verified in Resend
 * (the latter specifically for inbound reply detection on cold outreach -
 * see app/api/webhooks/resend-inbound).
 */
...
export async function sendColdOutreachEmail(step, { to, firstName, audience = "business" }) {
  ...
  return sendVia(to, templateFn({ firstName }), { fromAddress: OUTREACH_FROM_ADDRESS, replyTo: OUTREACH_REPLY_TO });
}
