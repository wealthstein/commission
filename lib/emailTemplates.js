...
export function coldOutreach1Template({ firstName, companyName }) {
  return {
    subject: `quick question about how ${companyName} finds new customers`,
    html: wrap(
      heading("A quick question") +
        paragraph(`Hi ${firstName}, curious how ${companyName} is currently finding new customers beyond word of mouth.`) +
        paragraph(
        ...
        button("See how it works", "https://www.commission.ng/calculator?for=business")
    ),
  };
}

export function coldOutreach2Template({ firstName, companyName }) {
  return {
    subject: "the risk with ads is you pay whether or not it works",
    html: wrap(
      heading("You pay either way with ads") +
        paragraph(
          `Hi ${firstName}, most ad spend is paid upfront, whether or not it turns into a customer. Commission flips that - you only pay when a real qualified lead or sale happens, and you're the one who sets what that's worth.`
        ) +
        paragraph("No ad account needed, no bidding against competitors for the same keywords.") +
        button("See if this fits", "https://www.commission.ng/calculator?for=business")
    ),
  };
}

export function coldOutreach3Template({ firstName, companyName }) {
  return {
    subject: "how the affiliate side actually works",
    html: wrap(
      heading("The short version") +
        paragraph(
          `Hi ${firstName}, you list what you're selling, affiliates - people who already have an audience that trusts them - share a link, and when it converts, they earn a commission automatically. Up to 3 tiers deep, so an affiliate is even rewarded for bringing in other affiliates.`
        ) +
        paragraph("Setting up a campaign takes a few minutes, not a new ad account and a learning curve.") +
        button("Open to a look?", "https://www.commission.ng/calculator?for=business")
    ),
  };
}

export function coldOutreach4Template({ firstName, companyName }) {
  return {
    subject: "in case cost or setup was the hesitation",
    html: wrap(
      heading("No fixed ad budget required") +
        paragraph(
          `Hi ${firstName}, if this hasn't been a priority for ${companyName}, that's completely fair. Just flagging that Commission's fee only applies to your Campaign Wallet top-up - nothing further is owed unless a lead or sale actually happens.`
        ) +
        button("See real examples", "https://www.commission.ng/calculator?for=business")
    ),
  };
}

export function coldOutreach5Template({ firstName, companyName }) {
  return {
