        ...
        <table role="presentation" width="100%" style="max-width:480px;" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td style="border-top:1px solid ${BORDER}; padding-top:24px;">
              <p style="margin:0; font-size:11px; color:${MUTED}; text-align:center;">© ${new Date().getFullYear()} Commission</p>
            </td>
          </tr>
        </table>
      </td>
...
// ---------------------------------------------------------------------------
// OUTBOUND / GROWTH CAMPAIGNS - manually-sent sequences to prospects who
// are not yet Commission users at all (cold B2B outreach, affiliate
// referral canvassing). Distinct from the transactional/marketing/revenue
// templates above, which only fire for existing accounts.
// ---------------------------------------------------------------------------

// Step 1 of 3 - cold outreach to a business decision-maker who has never
// heard of Commission. Leads with the single clearest differentiator
// (pay only for results) rather than a feature list.
export function coldOutreachStep1Template({ firstName, companyName, industryName, ppqlNaira, signupUrl }) {
  return {
    subject: `Quick question about ${companyName}'s customer acquisition`,
    html: wrap(
      heading(`Hi ${firstName || "there"}`) +
        paragraph(
          `I run Commission, a performance-based customer acquisition platform for Nigerian businesses. ${
            industryName ? `Other ${industryName} businesses` : "Businesses like yours"
          } are paying around <strong>${naira(ppqlNaira || 5000)} per Intent Qualified Lead</strong> - and nothing at all until one actually happens.`
        ) +
        paragraph(`No ad spend, no bidding war, no retainer. You set the price, affiliates share your link, you only pay for a real result.`) +
        button("See how it works", signupUrl)
    ),
  };
}

// Step 2 - sent a few days later if step 1 got no reply. Addresses the
// most common objection (skepticism about a new platform) with mechanics,
// not persuasion - how proceeds/payouts actually move.
export function coldOutreachStep2Template({ firstName, companyName, calculatorUrl }) {
  return {
    subject: `Following up - how the money actually moves`,
    html: wrap(
      heading(`Hi ${firstName || "there"} again`) +
        paragraph(
          `Following up in case the first email got buried. The part worth understanding before anything else: Commission never touches your customers' payments - Paystack splits everything automatically, your proceeds route straight to your own account, same as any transaction you already process.`
        ) +
        paragraph(`Takes about 10 minutes to see what a campaign for ${companyName} could look like.`) +
        button("Run the numbers", calculatorUrl)
    ),
  };
}

// Step 3 - final, short, low-pressure. Gives a clean exit rather than
// escalating pressure, which tends to convert better on the margin anyway.
export function coldOutreachStep3Template({ firstName, signupUrl }) {
  return {
    subject: `Last one from me`,
    html: wrap(
      heading(`No hard feelings, ${firstName || "there"}`) +
        paragraph(`I'll stop following up after this - if a pay-only-for-results channel is ever worth a look, the door's open.`) +
        button("List your first campaign free", signupUrl)
    ),
  };
}

// Affiliate-referral canvassing - encourages an EXISTING affiliate to
// refer other affiliates into their own downline (tier 2/3 of the
// commission structure), not a business decision-maker.
export function affiliateReferralCanvassTemplate({ firstName, referralLink, tier2Percent }) {
  return {
    subject: `Earn from every affiliate you bring in, not just your own sales`,
    html: wrap(
      heading("You can earn twice over") +
        paragraph(
          `Hi ${firstName}, every affiliate you refer into Commission puts you a tier above them - you earn ${
            tier2Percent ? `up to ${tier2Percent}%` : "a share"
          } of what they generate, automatically, for as long as they're active. No extra work after the introduction.`
        ) +
        detailRows([["Your referral link", referralLink]]) +
        button("Copy your referral link", referralLink) +
        paragraph("Share it with anyone who already promotes things to an audience - the tier structure does the rest.")
    ),
  };
}
...
export const EMAIL_TEMPLATES = {
  ...
  coldOutreachStep1: coldOutreachStep1Template,
  coldOutreachStep2: coldOutreachStep2Template,
  coldOutreachStep3: coldOutreachStep3Template,
  affiliateReferralCanvass: affiliateReferralCanvassTemplate,
};
