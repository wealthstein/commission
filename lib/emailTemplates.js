...
function outreachButton(text, url) {
  return `<tr><td style="padding-bottom:8px;"><a href="${url}" style="display:inline-block; background-color:${INK}; color:#FFFFFF; text-decoration:none; font-weight:600; font-size:15px; padding:14px 28px; border-radius:6px;">${text}</a></td></tr>`;
}

function outreachSignature() {
  return `<tr><td style="padding-top:32px; margin-top:8px; border-top:1px solid ${BORDER};"><table role="presentation" cellpadding="0" cellspacing="0"><tr><td style="padding-top:24px; font-size:16px; font-weight:600; color:${INK};">Wealthstein Adekoya</td></tr><tr><td style="font-size:14px; color:${MUTED};">Founder, Commission</td></tr></table></td></tr>`;
}
...
export function coldOutreach1Template({ firstName }) {
  return {
    subject: "Quick question about how you're finding new customers",
    html: outreachWrap(
      outreachBadge("Partnership Opportunity") +
        outreachHeading("A quick question") +
        outreachParagraph(`Hi ${firstName},`) +
        outreachParagraph(`Curious how you're currently finding new customers beyond word of mouth.`) +
        outreachCallout(
          "Commission is built so businesses only pay when something actually happens - an Intent Qualified Lead or a verified sale, never for clicks or impressions. You set the price, affiliates share your link, commissions get paid out automatically."
        ) +
        outreachButton("See how it works", "https://www.commission.ng/") +
        outreachSignature(),
      "You only pay when an Intent Qualified Lead or sale actually happens - never for clicks."
    ),
  };
}
  ...
  return {
    ...
    html: outreachWrap(
      outreachBadge("Partnership Opportunity") +
        ...
        outreachParagraph(
          `Hi ${firstName}, most ad spend is paid upfront, whether or not it turns into a customer.`
        ) +
        outreachCallout(
          "Commission flips that - you only pay when a real Intent Qualified Lead or sale happens, and you're the one who sets what that's worth."
        ) +
        ...
        outreachSignature(),
      "No ad account needed, no bidding against competitors for the same keywords."
    ),
  };
}
  ...
  return {
    ...
    html: outreachWrap(
      outreachBadge("Partnership Opportunity") +
        ...
        outreachParagraph(
          `Hi ${firstName}, you list what you're selling, affiliates - people who already have an audience that trusts them - share a link, and when it converts, they earn a commission automatically.`
        ) +
        outreachCallout(
          "Up to 3 tiers deep, so an affiliate is even rewarded for bringing in other affiliates."
        ) +
        ...
        outreachSignature(),
      "Up to 3 tiers deep - an affiliate is even rewarded for bringing in other affiliates."
    ),
  };
}
  ...
  return {
    ...
    html: outreachWrap(
      outreachBadge("Partnership Opportunity") +
        ...
        outreachParagraph(`Hi ${firstName}, if this hasn't been a priority, that's completely fair.`) +
        outreachCallout(
          "Just flagging that Commission's fee only applies to your Campaign Wallet top-up - nothing further is owed unless an Intent Qualified Lead or sale actually happens."
        ) +
        outreachButton("See real examples", "https://www.commission.ng/signup?role=business") +
        outreachSignature(),
      "Nothing is owed unless an Intent Qualified Lead or sale actually happens."
    ),
  };
}
  ...
  return {
    ...
    html: outreachWrap(
      outreachHeading("I'll leave it here") +
        outreachParagraph(
          `Hi ${firstName}, didn't want to keep following up without a clear signal this is useful. If customer acquisition becomes a priority down the line, commission.ng has the details whenever it's relevant.`
        ) +
        ...
        outreachSignature(),
      "Didn't want to keep following up without a clear signal this is useful."
    ),
  };
}
