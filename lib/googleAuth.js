 ...
 * for anyone whose access_granted is not yet true - which is
 * everyone, pre-launch. Never hardcode next to /welcome here; that would
 * incorrectly block a real, already-granted sign-in too.
 */
export async function triggerGoogleAuth({ role, sourcePage, flow, next } = {}) {
  const supabase = createClient();
  const params = new URLSearchParams({ next: next || urls.dashboard() });
