 ...
 * conversation belongs to — without needing any separate lookup. The
 * message also includes a direct link to the Intent Form (the fuller
 * details form, with any custom questions) - this was missing entirely
 * before, leaving the customer no way to reach it unless the business
 * manually sent it during the chat.
 */
export function buildLeadWhatsAppLink({ whatsappNumber, whatsappRef, productName, leadName }) {
  const intentFormUrl = `${SITE_URL}/leads/${whatsappRef}/continue`;
  const message = `Hi, I am ${leadName} and I am interested in ${productName}. (Ref: ${whatsappRef})\n\nI'll also share a few more details here: ${intentFormUrl}`;
