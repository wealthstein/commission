...
/**
 * Parses the OLD flat-slug URL patterns this site used before some pages
 * moved to nested folders. Used ONLY by app/[slug]/page.js for legacy
 * redirects — comparisons now live at their own bare root URL (e.g.
 * /google-ads, handled directly, not through this parser at all):
 *   /commission-and-google-ads   -> redirects to /google-ads
...
/** Metadata for a company or industry program page (formerly "affiliate program"). */
export function buildSeoTargetMetadata(target) {
  const canonical = `${SITE_URL}/programs/${target.route_slug}`;
  const title =
    target.type === "company"
      ? `${target.display_name} Program \u2014 Is There One? | Commission`
      : `${target.display_name} Programs in Nigeria | Commission`;
  const description =
    target.meta_description ||
    (target.type === "company"
      ? `Looking for a Commission program from ${target.display_name}? See what is currently available and get notified the moment one launches.`
      : `Browse ${target.display_name} programs in Nigeria and start earning commission with Commission.`);
...
export function buildSeoTargetFaqJsonLd(target, hasLiveProducts) {
  const answer = hasLiveProducts
    ? `Yes — ${target.display_name} has an active program on Commission. See it for details.`
    : `Not yet on Commission. You can request early access and get notified the moment ${target.display_name} (or a similar ${target.industry_category || "business"}) launches a program here.`;
  return {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: [
      {
        "@type": "Question",
        name: `Does ${target.display_name} have a program on Commission?`,
