...
export function buildProductMetadata({ product, business, isCustomerVisit = false }) {
  const title = isCustomerVisit
    ? `${product.name} — ${business.name} • Commission`
    : `${product.name} Affiliate Program \u2014 Earn Commission Promoting ${business.name} • Commission`;
  const description = isCustomerVisit
    ? `${product.name}, offered by ${business.name}. ${
        product.description ? product.description.slice(0, 140) : `${product.category || "Product"} available in Nigeria.`
      }`.trim()
    : `Promote ${product.name} by ${business.name} and earn commission on every sale. ${
